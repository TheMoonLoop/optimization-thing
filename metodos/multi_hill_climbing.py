...
def hill_climbing(funcion, x0, sigma, pasos):
    x = np.array(x0)
    valor = funcion(x)
    ruta = [x.copy()]
    historial = [valor]
    ...
    return x, historial, ruta

def run(funcion, intervalo):
    st.subheader("Método de Hill Climbing")

    resultado = configurar_parametros(funcion, intervalo, clave_prefix="hill_")
    ...
    mejor, historial, ruta = hill_climbing(
